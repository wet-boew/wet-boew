					...
					objEventsList.children('li').each(function () {
						...
						*/
						if (tCollection.length > 1) {
							...
							strCustClass = ($(tCollection[0]).get(0).nodeName.toLowerCase() === 'time' && $(tCollection[0]).hasOwnProperty('custClass') ) ? $(tCollection[0]).attr('custClass') : 'calEvent';
							//prePend = ($(tCollection[0]).get(0).nodeName.toLowerCase() === 'time') ? $(tCollection[0]).attr('custClass') : 'calEvent';
							...
							for (z = 0, _zlen = pe.date.daysBetween(strDate1, strDate2) + 1; z <= _zlen; z += 1) {
								...
								events.list[events.iCount] = { 'title': title, 'date': new Date(date.getTime()), 'href': link, 'custclass' : strCustClass };
							...
							strCustClass = ($(tCollection[0]).get(0).nodeName.toLowerCase() === 'time' && $(tCollection[0]).attr('custClass') !== undefined ) ? $(tCollection[0]).attr('custClass') : 'calEvent';
							...
							events.list[events.iCount] = {'title' : title, 'date' : date, 'href' : link, 'custclass' : strCustClass};
							// add a viewfilter
							if (!_objTitle.hasClass('filter-' + (date.getFullYear()) + '-' + pe.string.pad(date.getMonth() + 1, 2))) {
								_objTitle.addClass('filter-' + (date.getFullYear()) + '-' + pe.string.pad(date.getMonth() + 1, 2));
							}
				...
				for (e = 0, _elen = eventslist.length; e !== _elen; e += 1) {
					...
					if (date.getMonth() === month && date.getFullYear() === year) {
						...
						if (day.children('a').length < 1) {
							//alert(eventslist[e].pre);
							day.empty();
							link = $('<a href="#ev-' + day.attr('id') + '" class="' + eventslist[e].custclass + '">' + content + '</a>');
							alert("The current link = " + link);
							day.append(link);

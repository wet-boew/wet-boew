	...
	_wet_boew_theme = {
		...
		mobileview: function () {
			...
			if (wet_boew_theme.menubar.length > 0 || pe.secnav.length > 0 || wet_boew_theme.search.length > 0) {
				if (force_dialog) { // Fallback for mobile devices that can't handle the popup with a lot of nested menus
					popup_role = 'data-role="page"';
					popup_linktype = 'dialog';
					popup_close = '';
					popup_parent = pe.container();
				}

				// Transform the menu to a popup
				mb_li = wet_boew_theme.menubar.find('ul.mb-menu li');
				mb_dialogue = '<div ' + popup_role + ' id="jqm-wb-mb"><div data-role="header">';
				...
				popup_parent.append(mb_dialogue);
			...
			if (wet_boew_theme.search.length > 0) {
				...
				popup_parent.append(s_dialogue);
				//srch_head.wrapInner('<a href="#jqm-wb-search" data-rel="' + popup_linktype + '"></a>');

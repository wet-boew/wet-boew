		...
		mobileview: function () {
			...
			if (wet_boew_theme.sft.length !== 0) {
				...
				if (wet_boew_theme.wmms.length !== 0) {
					...
					header += '<div class="ui-title"><object type="image/svg+xml" width="90" data="' + node.getAttribute('src').replace('.gif', '-r.svg') + '"><img src="' + node.getAttribute('src').replace('.gif', '-wm.gif') + '" width="90" alt="' + node.getAttribute('alt') + '" /></object></div>';
				} else {

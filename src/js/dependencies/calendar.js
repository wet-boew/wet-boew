	...
	_pe.fn.calendar = {
		...
		createMonthNav : function (calendarid, year, month, minDate, maxDate) {
			...
			for (n = 0; n < 2; n += 1) {
				...
				if (showButton) {
					alt = titleSuffix + _pe.dic.get('%calendar-monthNames')[newMonth] + ' ' + newYear;
		...
		yearChanged : function (event) {
			...
			for (i = minMonth; i <= maxMonth; i += 1) {
				monthField.append('<option value="' + i + '"' + ((i === month) ? ' selected="selected"' : '') + '>' + _pe.dic.get('%calendar-monthNames')[i] + '</option>');
			}
		},
		...
		createGoToForm : function (calendarid, year, month, minDate, maxDate) {
			...
			if (_pe.ie === 6) {
				$(_pe.dic.get('%calendar-monthNames')).each(function (index, value) {
			...
			for (week = 1; week < 7; week += 1) {
				for (day = 0; day < 7; day += 1) {
					if ((week === 1 && day < firstday) || (daycount > lastday)) {
						...
						if (_pe.language === 'en') {
							child.append('<span class="wb-invisible">' + _pe.dic.get('%calendar-weekDayNames')[day] + ' ' + _pe.dic.get('%calendar-monthNames')[month] + ' </span>' + daycount + '<span class="wb-invisible"> ' + year + ((isCurrentDate) ? _pe.dic.get('%calendar-currentDay') : '') + '</span>');
						} else if (_pe.language === 'fr') {
							child.append('<span class="wb-invisible">' + _pe.dic.get('%calendar-weekDayNames')[day] + ' </span>' + daycount + '<span class="wb-invisible"> ' + _pe.dic.get('%calendar-monthNames')[month].toLowerCase() + ' ' + year + ((isCurrentDate) ? _pe.dic.get('%calendar-currentDay') : '') + '</span>');
						}

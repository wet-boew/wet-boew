	...
	_pe.fn.chartsGraph = {
		...
		generate: function (elm) {
			var options = {},
				...
				colorsAccent = ["#8d201c", "#EE8310", "#2a7da6", "#5a306b", "#285228", "#154055", "#555555", "#f6d200", "#d73d38", "#418541", "#87aec9", "#23447e", "#999999"]; // Default Colors set
			...
			function classToJson (el, namespace) {
				var strClass = "";
				if (typeof (el) === 'string') {
					strClass = el;
				} else {
				...
				return setClassOptions(jQuery.extend(true, o.optionsClass, o.axis2dgraph), strClass, namespace);
			}
			...
			function CalculateVerticalTick(parsedData) {
				...
				for (i = 0; i < parsedData.colgrouphead.col[0].cell.length; i += 1) {
					...
					var parsedDataCell = parsedData.colgrouphead.col[0].cell[i];
					...
					if (i === 0 || (i > 0 && parsedData.colgrouphead.col[0].cell[i - 1].uid !== parsedDataCell.uid)) {
						...
						if (parsedDataCell.type === 1 || parsedDataCell.type === 7)  {
							...
							function AddMultiplierFactor(parsedDataCell2){
								var kIndex;
								/*if (parsedDataCell2.child.length === 0) {
									return;
								}*/
								headerlevel += 1
								tblMultiplier.push([parsedDataCell2.child.length, headerlevel]);
								for (kIndex = 0; kIndex < parsedDataCell2.child.length; kIndex += 1) {
									if (parsedDataCell2.child[kIndex].child.length > 0){
										AddMultiplierFactor(parsedDataCell2.child[kIndex]);
									}
								}
								headerlevel -= 1;
							}
				...
				var UseHeadRow = parsedData.colgrouphead.col.length - 1;
							...
							function AddMultiplierFactor(parsedDataCell){
								...
								for (kIndex = 0; kIndex < parsedDataCell.child.length; kIndex += 1) {
									...
									if (parsedDataCell.child[kIndex].child.length > 0){
										AddMultiplierFactor(parsedDataCell.child[kIndex]);
									}
								}
								headerlevel -= 1;
							}
			...
			function ChartPlacement(allSeries, ChartOptions, caption, description) {
				
				// Create the placeholder
				var placeHolder = $('<div />').insertAfter($(self));
				...
				$.plot(placeHolder, allSeries, ChartOptions);
			}
			...
			var calcTick = horizontalCalcTick;
				...
				for (rIndex=parsedData.lstrowgroup[0].row.length - 1; rIndex >= 0; rIndex -= 1) {
					...
					for (i=0;i<dataGroup.col.length; i += 1) {
						...
						for(j=0; j<dataGroup.col[i].cell.length; j += 1){
							...
							// Get's the value
							var header = dataGroup.col[i].cell[j].row.header;
						...
						allSeries.push({ data: dataSeries, label: $(dataGroup.col[i].dataheader[dataGroup.col[i].dataheader.length - 1].elem).text(), color: colorsAccent[i]});
					}
			...
			for (i=0;i<parsedData.lstrowgroup[0].row.length; i++) {
				var rowOptions = setClassOptions(RowDefaultOptions,				
			...
			for (i=0;i<parsedData.lstrowgroup[0].row.length; i++) {
				...
				var rowOptions = parsedData.lstrowgroup[0].row[i].header[parsedData.lstrowgroup[0].row[i].header.length - 1].chartOption;

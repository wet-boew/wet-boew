		...
		generate: function (elm) {
			...
			if (o.type === "pie") {
				...
				for (rIndex=parsedData.lstrowgroup[0].row.length - 1; rIndex >= 0; rIndex -= 1) {
					...
					for (i=0;i<dataGroup.col.length; i += 1) {
						...
						allSeries.push({ data: dataSeries, label: $(dataGroup.col[i].dataheader[dataGroup.col[i].dataheader.length - 1].elem).text(), color: colourNameToHex(i)});
					}
			...
			for (i=0;i<parsedData.lstrowgroup[0].row.length; i++) {
				...
				if (rowOptions.type === "line") {
					allSeries.push({ data: dataSeries, label: $(parsedData.lstrowgroup[0].row[i].header[parsedData.lstrowgroup[0].row[i].header.length - 1].elem).text(), color: colourNameToHex(i)});

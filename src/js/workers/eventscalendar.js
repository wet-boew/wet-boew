...
(function ($) {
	...
	_pe.fn.eventscalendar = {
		...
		_exec: function (elm) {
			...
			events = _pe.fn.eventscalendar.getEvents(elm);
			...
			$("#" + containerid).bind("calendarDisplayed", function (e, year, month, days) {
				pe.fn.eventscalendar.addEvents(year, month, days, containerid, events.list);
				pe.fn.eventscalendar.showOnlyEventsFor(year, month, containerid);
			});
		...
		getEvents: function (obj) {
			...
			if (objEventsList.length > 0) {
				objEventsList.children("li").each(function (e) {
					...
					* ***/
					if (!direct_linking) {
						link_id = (event.attr('id') !== undefined) ? event.attr('id') : pe.fn.eventscalendar.randomId(6);

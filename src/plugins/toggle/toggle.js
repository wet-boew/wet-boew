		...
		plugin = {
			...
			init: function() {
				...
				if ( link.data( "selector" ) !== undefined  ) {
					...
					$.error( ".wb-toggle: you must specify a [data-selector] attribute with the CSS selector of the element(s) the toggle link controls." );
				}
			},
			...
			setState: function( selector, parent, state ) {
				...
				if ( plugin.state[ selector ] === undefined ) {
					plugin.state[ selector ] = { all: plugin.stateOff };
				}

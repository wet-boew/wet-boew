...
const componentName = "wb-tagfilter",
	...
	updateStatusMessage = function( instance ) {
		...
		if ( statusWrapper && statusMessageElm ) {
			...
			//  Note: Since the no result wrapper text is not dynamic, it wouldn't get announced by screen readers when the filter changes. This is why we copy it to the status message element.
			if ( matchedCount === 0 ) {

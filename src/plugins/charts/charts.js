...
// TODO: Enable details/summary polyfill
...
var _pe,
	...
	generate: function (elm) {
			...
			function setClassOptions (sourceOptions, strClass, namespace) {
				...
				$.each(arrClass, function () {
					...
					} else if (namespace === (this.length > namespace.length + separatorNS.length ? this.slice(0, namespace.length) : "")) {
	...
	init: function() {
		...
		var meThis = this,
			...
			deps = [ "site!deps/jquery.flot" + modeJS,
					 "site!deps/jquery.flot.pie" + modeJS,
					 "site!deps/jquery.flot.canvas" + modeJS ];
			
		if (!plugin.flotloaded) {
			// Load the required dependencies and prettify the code once finished
			window.Modernizr.load({
				//load: ["site!deps/jquery.flot.js"],
				
				// ADD instruction in Gruntfile.cofee near line  444 (Check for magnefic) 
	...
	tableparsed: function(event) {
		vapour.doc.trigger( {type: "readytodraw.wb-charts", pointer: event.pointer} );
	},
	
	start: function(event) {
		plugin.generate(event.pointer);
	}
};
...
vapour.doc.on( "timerpoke.wb", plugin.selector, plugin.init )
	.on( "readytodraw.wb-charts", plugin.start )
	.on( "parsecomplete.wb-table.wb", plugin.tableparsed );

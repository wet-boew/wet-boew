...
var selector = ".wb-parse",
	$document = vapour.doc;


function exectableparser(obj){
	...
	*/
	...
	$document.trigger( {type: "parsecomplete.wb-table.wb", pointer: obj} );
                
}



$document.on( "error.wb-table.wb", function (event) {
	$(event.pointer).css("color", "red");
});


// Bind the plugin events
$document.on( "timerpoke.wb", selector, function () {
		exectableparser(this);
	}).
	on( "pasiveparse.wb-table.wb", function (event) {
		exectableparser(event.pointer);
	}) ;

...
const componentName = "wb-paginate",
	...
	init = function( event ) {
		...
		if ( elm ) {
			...
			paginationElm.setAttribute( "aria-label", "Pagination navigation" );
	...
	generateUI = function( elm ) {
		...
		if ( pagesCount > 1 ) {
			...
			prevLI += "<button type=\"button\" class=\"paginate-prev\" aria-controls=\"" + elm.id + "\">" + i18nText.prv + "<span class=\"wb-inv\"> Page</span></button>";
			prevLI += "</li>";
			...
			nextLI += "<button type=\"button\" class=\"paginate-next\" aria-controls=\"" + elm.id + "\">" + i18nText.nxt + "<span class=\"wb-inv\"> Page</span></button>";
			nextLI += "</li>";
			paginationUI += nextLI;
			paginationUI += "</ol>";
		}

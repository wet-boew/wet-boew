	...
	_wet_boew_theme = {
		...
		mobileview: function () {
			...
			if (wet_boew_theme.sft.length !== 0) {
				...
				// Append the header
				wet_boew_theme.gcnb.children('#wet-gcnb-in').before(header);
				...
				links = document.getElementById('wet-tctr').getElementsByTagName('a');
			...
			$(document).on('pagecreate', function () {
				...
				if (_list.length !== 0) {
					var navbar = wet_boew_theme.gcnb.find('#wet-mnavbar'),

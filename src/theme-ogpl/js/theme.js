...
(function ($) {
	...
	*/
	...
	_wet_boew_theme = {
		...
		init: function () {
			wet_boew_theme.fullhd = pe.header.find('#ogpl-fullhd');
			wet_boew_theme.psnb = pe.header.find('#ogpl-psnb');
			wet_boew_theme.menubar = wet_boew_theme.psnb.find('.wet-boew-menubar');
			wet_boew_theme.search = pe.header.find('#ogpl-srchbx');
			wet_boew_theme.bcrumb = pe.header.find('#ogpl-bc');
			wet_boew_theme.title = pe.header.find('#ogpl-title');
			wet_boew_theme.sft = pe.footer.find('#ogpl-sft');
			wet_boew_theme.fullft = pe.footer.find('#ogpl-fullft');
		...
		mobileview: function () {
			...
			if (wet_boew_theme.sft.length !== 0) {
				...
				// Add the version
				node = pe.main.find('#ogpl-date-mod').children();
				if (node.length !== 0) {
					target = node[1];
				...
				nodes = wet_boew_theme.sft.find('.ogpl-col-head');
				...
				lang_links = document.getElementById('ogpl-lang');
				if (lang_links !== null) {
					...
					lang_nav += '</ul></div>';
					document.getElementById('ogpl-other-lang').parentNode.innerHTML = lang_nav;
				}

				// Handling for the terms and conditions links
				target = document.getElementById('ogpl-tc');
				if (target !== null) {
			...
			$document.on('pagecreate', function () {
				var navbar = wet_boew_theme.fullhd.find('#ogpl-mnavbar'),

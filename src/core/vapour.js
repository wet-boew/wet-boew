...
(function ($, window, document, undef) {
    "use strict";
    var $src = $("script[src$='vapour.js'],script[src$='vapour.min.js']").last(),
        $homepath = $src.prop("src").split("?")[0].split("/").slice(0, -1).join("/"),
        $mode = $src.prop("src").indexOf(".min") < 0 ? "" : ".min",
        vapour = {
            "/": $homepath,
 ...
 */
    Modernizr.load([{
        ...
        test: navigator.userAgent.indexOf("Win") !== -1 && navigator.userAgent.match(/^((?!mobi|tablet).)*$/i) !== null,
